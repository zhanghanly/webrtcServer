...
namespace RTC
{
	  ...
	  : id(id), shared(shared), listener(listener)//, kind(RTC::Media::Kind(data->kind()))
	{
		...
		// This may throw.
		this->rtpParameters = RTC::RtpParameters(rtp_params);
		...
		for (const auto& item : rtp_params.rtpmapping().payloadmap()) {
			this->rtpMapping.codecs[item.payloadtype()] = item.mappedpayloadtype();
		}	
		
		for (const auto& rtpMapping : rtp_params.rtpmapping().encodingmap()) {
			//std::cout << __FUNCTION__ << " " << (int)rtpMapping.payloadtype() << " " << rtpMapping.mappedpayloadtype() << std::endl;
			//this->rtpMapping.codecs[rtpMapping.payloadtype()] = rtpMapping.mappedpayloadtype();
			//if (kind == RTC::Media::Kind::AUDIO) {
				RtpEncodingMapping encodingMapping;
				encodingMapping.rid = rtpMapping.rid();
				encodingMapping.ssrc = rtpMapping.ssrc();
				encodingMapping.mappedSsrc = rtpMapping.mappedssrc();
				this->rtpMapping.encodings.push_back(encodingMapping);
			//}
		}
		...
		this->paused = false;
		this->enableMediasoupPacketIdHeaderExtension = true;
		...
		if (this->kind == RTC::Media::Kind::VIDEO)
		{
			...
			auto keyFrameRequestDelay = 1000;
	...
	Producer::ReceiveRtpPacketResult Producer::ReceiveRtpPacket(RTC::RtpPacket* packet)
	{
		...
		//std::cout << __FUNCTION__ << " " << (int)(packet->GetPayloadType()) << " ssrc=" << packet->GetSsrc() << std::endl;
		...
		if (!rtpStream)
		{
			...
			//std::cout << "not found rtpStream " << (int)(packet->GetPayloadType()) << " ssrc=" << packet->GetSsrc() << std::endl;
	...
	RTC::RtpStreamRecv* Producer::GetRtpStream(RTC::RtpPacket* packet)
	{
		...
		{
			std::cout << __FUNCTION__ << " rid=" << rid << " ssrc=" << packet->GetSsrc() << " payloadType=" << (int)payloadType << std::endl;
			for (size_t i{ 0 }; i < this->rtpParameters.encodings.size(); ++i)
			{
				auto& encoding = this->rtpParameters.encodings[i];
		...
		std::cout << "not read rid" << std::endl;
	  ...
	  RTC::RtpPacket* packet, const RTC::RtpCodecParameters& mediaCodec, size_t encodingIdx)
	{
		...
		std::cout << "ssrc=" << ssrc << " encodingMappedSsrc=" << encodingMapping.mappedSsrc << std::endl;
