...
namespace RTC
{
	...
	WebRtcTransport::WebRtcTransport(
	  ...
	  const FBS::WebRtcTransport::WebRtcTransportOptions* options)
	  //: RTC::Transport::Transport(shared, id, listener, options->base()),
	  : RTC::Transport::Transport(shared, id, listener, nullptr),
	...
	server::CreateTransportResponse WebRtcTransport::CreateResponse(void) {
		std::vector<server::IceCandidate> candidates;
		for (const auto& iceCandidate : this->iceCandidates) {
			candidates.push_back(iceCandidate.DumpCandidate());
		}
		...
		for (const auto& fingerprint : RTC::DtlsTransport::GetLocalFingerprints()) {
			...
			if (fingerprint.algorithm == DtlsTransport::FingerprintAlgorithm::SHA1) {
				algorithm = "sha-1";
			} else if (fingerprint.algorithm == DtlsTransport::FingerprintAlgorithm::SHA224) {
				algorithm = "sha-224";
			} else if (fingerprint.algorithm == DtlsTransport::FingerprintAlgorithm::SHA256) {
				algorithm = "sha-256";
			} else if (fingerprint.algorithm == DtlsTransport::FingerprintAlgorithm::SHA384) {
				algorithm = "sha-384";
			} else if (fingerprint.algorithm == DtlsTransport::FingerprintAlgorithm::SHA512) {
				algorithm = "sha-512";
			}
			
			fingerprints.push_back(worker::ResponseBuilder::BuildDtlsFingerprint(
            algorithm, fingerprint.value));
			std::cout << algorithm << " " << fingerprint.value << std::endl;
		}
	
		std::cout << "name=" << this->iceServer->GetUsernameFragment()
		          << " passwd=" << this->iceServer->GetPassword() << std::endl;
		return worker::ResponseBuilder::BuildCreateTransportResponse(
		           transportId,
		           this->iceServer->GetUsernameFragment(),
		           this->iceServer->GetPassword(),
		           candidates,
		           fingerprints,
		           "auto",
		           true); 
	...
	void WebRtcTransport::connectTransport(const std::string& role, const server::DtlsFingerprint& fingerprint) {
		// Ensure this method is not called twice.
		if (this->connectCalled) {
			MS_THROW_ERROR("connect() already called");
		}
		...
		RTC::DtlsTransport::Role dtlsRemoteRole;
		// NOTE: Just take the first fingerprint.
		dtlsRemoteFingerprint.algorithm = DtlsTransport::FingerprintAlgorithm::SHA256;
		dtlsRemoteFingerprint.value = fingerprint.value();
		dtlsRemoteRole = RTC::DtlsTransport::Role::CLIENT;
	  ...
	  RTC::Consumer* /*consumer*/, RTC::RtpPacket* packet, const RTC::Transport::onSendCallback* cb)
	{
		...
		//std::cout << "webrtcTransport ssrc=" << packet->GetSsrc() << " payloadType=" << (int)packet->GetPayloadType()  << std::endl; 
