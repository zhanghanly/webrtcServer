...
namespace RTC
{
	...
	WebRtcServer::WebRtcServer(
	  RTC::Shared* shared,
	  const std::string& id,
	  const server::ListenInfo& listenInfo)
	  : id(id), shared(shared)
	{
		MS_TRACE();
		auto ip = listenInfo.ip();
		// This may throw.
		...
		//{
			announcedAddress = listenInfo.announced_address();
		//}
		//const bool exposeInternalIp = listenInfo.exposeInternalIp();
		const bool exposeInternalIp = true;

		RTC::Transport::SocketFlags flags;
		flags.ipv6Only     = listenInfo.ipv6only();
		flags.udpReusePort = listenInfo.udpreuseport();

		if (listenInfo.protocol() == "UDP")
		{
			// This may throw.
			...
			if (listenInfo.send_buffer_size() != 0)
			{
				udpSocket->SetSendBufferSize(listenInfo.send_buffer_size());
			}

			if (listenInfo.recv_buffer_size() != 0)
			{
				udpSocket->SetRecvBufferSize(listenInfo.recv_buffer_size());
			}
		...
		else if (listenInfo.protocol() == "TCP")
		{
			// This may throw.
			...
			if (listenInfo.send_buffer_size() != 0)
			{
				tcpServer->SetSendBufferSize(listenInfo.send_buffer_size());
			}

			if (listenInfo.recv_buffer_size() != 0)
			{
				tcpServer->SetRecvBufferSize(listenInfo.recv_buffer_size());
			}
	...
	{
		MS_TRACE();
		//std::cout << __FUNCTION__ << "recieve packet" << std::endl;
