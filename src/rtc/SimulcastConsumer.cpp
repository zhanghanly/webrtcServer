...
namespace RTC
{
	...
	SimulcastConsumer::SimulcastConsumer(
	  ...
	  const server::RtpParameters& data)
	  : RTC::Consumer::Consumer(
	      shared, id, producerId, mediaKind, listener, data, RTC::RtpParameters::Type::SIMULCAST)
	{
		MS_TRACE();
		...
		for (size_t idx{ 0u }; idx < this->consumableRtpEncodings.size(); ++idx)
		{
			...
			std::cout << "add ssrc= " << encoding.ssrc << " in mapMappedSsrcSpatialLayer" << std::endl;
		...
		//{
			...
			//this->preferredLayers.spatial  = static_cast<int16_t>(encoding.spatialLayers - 1);
			//this->preferredLayers.temporal = static_cast<int16_t>(encoding.temporalLayers - 1);
			this->preferredLayers.spatial  = 2;
			this->preferredLayers.temporal = 0;
		//}
	...
	{
		//std::cout << __FUNCTION__ << " simulcast consume send packet" << std::endl;
		//std::cout << "targetLayers=" << targetLayers.temporal << " " << targetLayers.spatial << std::endl;
		MS_TRACE();
		...
		if (!IsActive())
		{
			...
			std::cout << "consume is not alive" << std::endl;
		...
		if (this->targetLayers.temporal == -1)
		{
			...
			std::cout << "this->targetLayers.temporal == -1" << std::endl;
		...
		if (!this->supportedCodecPayloadTypes[payloadType])
		{
			...
			std::cout << "!this->supportedCodecPayloadTypes[payloadType]=" << int(payloadType) << std::endl;
		...
		else if (spatialLayer != this->currentSpatialLayer)
		{
			// NOTE: Don't drop the packet in the RTP sequence manager since this
			// packet doesn't belong to the current spatial layer.
			std::cout << "spatialLayer != this->currentSpatialLayer" << std::endl;
		...
		if (result != RTC::RtpStreamSend::ReceivePacketResult::DISCARDED)
		{
			...
			//std::cout << "OnConsumerSendRtpPacket ssrc=" << packet->GetSsrc() << " payloadType=" << (int)packet->GetPayloadType()  << std::endl; 
			// Send the packet.
