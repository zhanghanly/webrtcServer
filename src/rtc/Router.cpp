...
namespace RTC
{
	...
	Router::Router(RTC::Shared* shared, const std::string& id, WebRtcServer* server)
	  : id(id), shared(shared), webRtcServer(server)
	{
		MS_TRACE();
	...
	//void Router::createWebRtcServer(const server::ListenInfo& info) {
	//	webRtcServer = new WebRtcServer(nullptr, "", info);
	//}
		
	server::CreateTransportResponse Router::createTransport(const std::string& transportId) {
		//FBS::WebRtcTransport::WebRtcTransportOptions options;
		//std::vector<RTC::IceCandidate> iceCandidates = webRtcServer->GetIceCandidates(
		//  options.enableUdp(), options.enableTcp(), options.preferUdp(), options.preferTcp());
		std::vector<RTC::IceCandidate> iceCandidates = webRtcServer->GetIceCandidates(
		...
		// This may throw.
		auto* webRtcTransport = new RTC::WebRtcTransport(
		  this->shared, transportId, this, webRtcServer, iceCandidates, nullptr);

		// Insert into the map.
		this->mapTransports[transportId] = webRtcTransport;
		//auto dumpOffset = webRtcTransport->FillBuffer(request->GetBufferBuilder());
	
		return webRtcTransport->CreateResponse();
	}
		
	void Router::connectTransport(const std::string& transportId, const std::string& role, 
		                          const server::DtlsFingerprint& fingerprint) {
		if (this->mapTransports.find(transportId) != this->mapTransports.end()) {
			RTC::Transport* transport = this->mapTransports[transportId];
			RTC::WebRtcTransport* webRtcTransport = dynamic_cast<RTC::WebRtcTransport*>(transport);
			if (webRtcTransport) {
				webRtcTransport->connectTransport(role, fingerprint);
			}
		}
    }
		
	void Router::addProducer(const std::string& transportId, const std::string& producerId,
