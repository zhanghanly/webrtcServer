...
namespace RTC
{
	...
	server::IceCandidate IceCandidate::DumpCandidate(void) const {
		std::cout << "candidate: " << foundation << " " << priority
		          << " " << this->address << " " << this->port << std::endl;
		
		return worker::ResponseBuilder::BuildIceCandidate(
            foundation, 
			priority,
		    this->address,
			"udp", 
